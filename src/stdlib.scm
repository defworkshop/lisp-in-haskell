...
(define (max first . num-lst)
  (fold (lambda (old new) (if (> old new) old new)) first num-lst))

(define (min first . num-lst)
  (fold (lambda (old new) (if (< old new) old new)) first num-lst))
...
(define (assoc obj alist)
  (fold (mem-helper (curry equal? obj) car) # alist))
...
(define (filter-pos pos-pred lst)
  (reverse 
   (car 
    (foldl (lambda (acc x) (if (pos-pred (cdr acc))
                               '(,(cons x (car acc)) . ,(+ 1 (cdr acc)))
                               '(,(car acc) . ,(+ 1 (cdr acc)))))
           '(() . 1) lst))))
